...
app.get("/api/pub-packages", async (req, res) => {
  const getPubPackageData = async (
    ...
    try {
      // 2. Fetch metrics
      const metricsRes = await fetch(`https://pub.dev/api/packages/${pkgName}/metrics`);
      if (metricsRes.ok) {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to fetch metrics from pub.dev background services" });
  }
});
